...
const messageUpdate = (oldMessage, newMessage) => {
  if (!oldMessage?.mentions || !newMessage?.mentions)
    throw new Error(
      "Expected parameters 'oldMessage', 'newMessage' at position 1, 2"
    );
  if (
    oldMessage.author.bot ||
    (oldMessage.mentions.members.size === 0 &&
      oldMessage.mentions.roles.size === 0)
  )
    return false;

  const oldArray = [
    ...oldMessage.mentions.roles.map((x) => filter(x)),
    ...oldMessage.mentions.members.map((member) =>
      validate(member, newMessage)
    ),
  ];

  const newArray = [
    ...newMessage.mentions.roles.map((x) => filter(x)),
    ...newMessage.mentions.members.map((member) =>
      validate(member, newMessage)
    ),
  ];

  const mentions = oldArray.filter(
    (mention) =>
      !newArray.includes(mention) && mention?.toString()?.startsWith("<")
  );
...
const messageDelete = (message) => {
  ...
  if (
    message.author.bot ||
    (message.mentions.members.size == 0 && message.mentions.roles.size == 0)
  )
    return false;
